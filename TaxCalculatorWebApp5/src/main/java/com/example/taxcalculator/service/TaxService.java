...
public class TaxService {
    public TaxResponse calculateTax(TaxRequest request) {
        ...
        double taxableIncome = totalPackage - variablePay - nps - 50000; // Standard deduction
        double tax = regime.equals("NEW") ? calculateNewRegimeTax(taxableIncome) : calculateOldRegimeTax(taxableIncome);
        double rebate = taxableIncome <= 700000 ? tax : 0;
        tax -= rebate;
    ...
    private double calculateNewRegimeTax(double income) {
        double tax = 0; double[] slabs = {300000,600000,900000,1200000,1500000}; double[] rates = {0.05,0.10,0.15,0.20,0.30}; double prev=0;
        for(int i=0;i<slabs.length;i++){ if(income>slabs[i]){ tax+=(slabs[i]-prev)*rates[i]; prev=slabs[i]; } else { tax+=(income-prev)*rates[i]; return tax; }}
        if(income>slabs[slabs.length-1]) tax+=(income-slabs[slabs.length-1])*rates[rates.length-1]; return tax;
    }

    private double calculateOldRegimeTax(double income) {
        double tax = 0; double[] slabs = {250000,500000,1000000}; double[] rates = {0.05,0.20,0.30}; double prev=0;
        for(int i=0;i<slabs.length;i++){ if(income>slabs[i]){ tax+=(slabs[i]-prev)*rates[i]; prev=slabs[i]; } else { tax+=(income-prev)*rates[i]; return tax; }}
        if(income>slabs[slabs.length-1]) tax+=(income-slabs[slabs.length-1])*rates[rates.length-1]; return tax;
    }
}
